from model.AttenModel.AttenUnet_KD import Mymodel
...
def edge_detection(img):
    ...
    img = img.cpu().detach()
    imgfolder = img.chunk(img.size()[0], dim=0)
    chw_output = []
    for index in range(img.size()[0]):
        ...
        x = cv2.Sobel(hw, cv2.CV_16S, 1, 0)
        y = cv2.Sobel(hw, cv2.CV_16S, 0, 1)
        ...
        chw = torch.unsqueeze(dst, dim=0)

        chw_output.append(chw)
    bcwh_edge = torch.stack(chw_output, dim=0)
...
def train(epoch):
    ...
    for batch_idx, (inputs, lungs, medias, targets_u, targets_i, targets_s) in enumerate(dataloader):
        ...
        targets_edge = torch.cat((edge_detection(targets_u), edge_detection(targets_i), edge_detection(targets_s)), dim=1)
        ...
        outputs_sig_edgemap = torch.sigmoid(outputs[4])
        
        targets_edge_flat = targets_edge.view(targets_edge.size(0), -1)
        outputs_edgemap_flat = outputs_sig_edgemap.view(outputs_sig_edgemap.size(0), -1)
        targets_edge_flat = targets_edge_flat.to(torch.float32)
        ...
        loss_edge = criterion_m(outputs_edgemap_flat, targets_edge_flat)

        # distillation
        IM_inputs, Channel_inputs = model(inputs, flag = True)
        IM_medias, Channel_medias = model(medias, flag = True)

        # caculate importance map loss
        losslist_im_media = []
        for i in range(5):
            loss_i  = criterion_im(IM_medias[i], IM_inputs[i])
            losslist_im_media.append(loss_i)
        loss_im_media = sum(losslist_im_media)

        # caculate channel alignment loss
        losslist_channel_kd = []
        gt = torch.cat((targets_i, targets_u), dim=1)
        gt = torch.cat((gt, targets_s), dim=1)
        for j in range(2):
            loss_j  = criterion_kd(Channel_inputs[j], Channel_medias[j], gt, 3)
            losslist_channel_kd.append(loss_j)
            loss_j  = criterion_kd(Channel_medias[j], Channel_inputs[j], gt, 3)
            losslist_channel_kd.append(loss_j)
        loss_channel_media = sum(losslist_channel_kd)

        loss_all = config.weight_seg1 * loss_seg_final + config.weight_seg2 * (loss_seg_i + loss_seg_u + loss_seg_s) + config.weight_edge * loss_edge + config.weight_kd * (loss_channel_media + loss_im_media)
