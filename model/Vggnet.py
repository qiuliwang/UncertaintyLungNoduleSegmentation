...
class VGG16Net(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.conv_f1 = nn.Conv2d(1, 3, 3, padding=1)
        self.conv_f2 = nn.Conv2d(3, 3, 3, padding=1)

        # channel alignment
        self.conv_channelalig1 = nn.Conv2d(3, 3, 3, padding=1)
        self.conv_channelalig2 = nn.Conv2d(128, 3, 3, padding=1)
        self.conv_channelalig3 = nn.Conv2d(512, 3, 3, padding=1)
    ...
    def forward(self, x, coarse_pred, flag1=False, flag2=True):
        ''' Encoder '''
        if len(coarse_pred)!=0:
            for y in coarse_pred:
                # y = coarse_pred[i]
        ...
        if flag2:
            ...
            x5 = F.interpolate(x5, size=(12,12), mode='bilinear', align_corners=True)
            x5 = x4+x5

            x = torch.cat((x1, x2, x3, x4, x5), dim=1)
            x = self.relu(self.bn1(self.conv1(x)))      # [b, 256, 32, 32]
            x = F.interpolate(x, size=(24,24), mode='bilinear', align_corners=True)
        ...
        if flag1:
            return [o1, o2, o3, o4, o5, o6], [low_ckd, mid_ckd, high_ckd]
        else:
